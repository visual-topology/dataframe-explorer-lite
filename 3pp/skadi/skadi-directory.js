...
skadi.DirectoryApi = class extends skadi.Api {
    ...
    constructor(element_id, options, plugins) {
        super();
        this.root_element = skadi.$$$(element_id);
        this.root_list = null;
        this.open_topology_in_designer_callback = null;
        this.create_topology_callback = null;
        this.remove_topology_callback = null;
        this.topologies = {};
    ...
    async load() {
        ...
        this.topology_store = this.plugins.topology_store || new skadi.TopologyStore();
        await this.topology_store.init();
        this.set_new_topology_handler((new_topology_id) => {
            this.topology_store.create_topology(new_topology_id);
        });

        this.set_remove_topology_handler((topology_id) => {
    ...
    set_new_topology_handler(callback) {
        this.create_topology_callback = callback;
    }

    set_remove_topology_handler(callback) {
        this.remove_topology_callback = callback;
    }

    can_open(topology_id) {
        let metadata = this.topology_metadata[topology_id];
        let topology_packages = this.topology_packages[topology_id];
        for(let idx=0; idx<topology_packages.length; idx++) {
            if (!this.schema.has_package_type(topology_packages[idx])) {
                return false;
            }
        }
        return true;
    ...
    draw(initial_topologies) {
        ...
        let create_fieldset = control_row.append("fieldset");
        ...
        this.create_topology_button.on("click", () => {
            let topology_id = this.create_name_input.node().value;
            if (!(topology_id in this.topology_metadata)) {
                if (this.create_topology_callback) {
                    this.create_topology_callback(topology_id);
                }
            }
    ...
    remove_topology(topology_id) {
        ...
        if (topology_id in this.topology_metadata) {
            ...
            if (this.remove_topology_callback) {
                this.remove_topology_callback(topology_id);
            }
        }
    }
    ...
    render_topology(topology_id) {
        ...
        if (this.open_topology_in_designer_callback && this.can_open(topology_id)) {
        ...
        if (pmetadata) {
            ...
            if (keys.length>1) {
                ...
                keys.forEach((key) => {
                    ...
                    tr.append("td").text(key + ":").style("border","none");
                    tr.append("td").text(pmetadata[key]).style("border","none");
                });
            }
        }
