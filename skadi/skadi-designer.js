...
skadi.InstanceFactory = class {
    ...
    async create_node_instance(node_id, node_type_id, classname, service) {
        return new skadi.GraphWorkerNodeWrapper(this.graph_worker_client, node_id, node_type_id, classname);
    }

    async create_configuration_instance(package_id, classname, service) {
        return new skadi.GraphWorkerConfigurationWrapper(this.graph_worker_client, package_id, classname);
    }
}
...
skadi.Core = class {
    ...
    constructor(workspace_id, id, l10n_utils, schema, instance_factory, topology_store, platform_extensions) {
        this.workspace_id = workspace_id;
        this.id = id;
...
skadi.AboutDialogue = class {
    ...
    constructor(design, elt) {
        this.about_link_text = design.localise("about.link_text");
        this.about_name = design.localise("about.name");
        this.about_version = design.localise("about.version");
        this.about_license_name = design.localise("about.license_name")

        let rowdata_platform = [
            [this.tn(this.about_name), this.tn(this.about_version), this.tn(this.about_license_name), this.tn(this.about_link_text)],
            [this.tn("Skadi"), this.tn(skadi.VERSION), this.tn("MIT"), this.mklink(this.about_link_text, "https://github.com/visual-topology/skadi")]
        ];

        try {
            let cls = zip.ZipReader;
            rowdata_platform.push([this.tn("zip.js"), this.tn("v2.7.52"), this.tn("BSD-3-Clause"), this.mklink(this.about_link_text, "https://github.com/gildas-lormeau/zip.js")])
        } catch(ex) {
            /* zip.js is not installed */
        }

        design.core.platform_extensions.forEach(o => {
            rowdata_platform.push([this.tn(o["name"]), this.tn(o["version"]), this.tn(o["license_name"]), o["url"] ? this.mklink(this.about_link_text, o["url"]) : this.tn("")]);
        });
        ...
        package_types.forEach(package_type_id => {
            ...
            rowdata_packages.push([this.tn(metadata.name),
                    ...
                    this.mklink(this.about_link_text, pt.get_resource_url(metadata.link))
            ]);
        });
...
skadi.Design = class  {
    ...
    constructor(core, canvas_div, canvas_width, canvas_height, is_acyclic) {
        this.core = core;
...
skadi.DesignerApi = class extends skadi.Api {
    ...
    async init(topology_id, element_id, options, plugins) {
        ...
        let engine = plugins.engine || new skadi.GraphWorkerClient(options["worker_path"]);
        let instance_factory = plugins.instance_factory || new skadi.InstanceFactory(engine);
        let topology_store = plugins.topology_store || new skadi.TopologyStore(this.workspace_id || "skadi_default_workspace");
        ...
        let schema = await this.load_schema(options.schema_urls, null);
        let core = new skadi.Core(this.workspace_id, topology_id, l10n_utils, schema, instance_factory, topology_store, options.platform_extensions);
...
skadi.PageFrame = class {
    ...
    constructor(target, page_id, title, url, parent_div, show_title, show_status) {
        this.target = target;
...
skadi.GraphWorkerWrapper = class {
...
skadi.GraphWorkerNodeWrapper = class extends skadi.GraphWorkerWrapper{
...
skadi.GraphWorkerConfigurationWrapper = class extends skadi.GraphWorkerWrapper {
...
skadi.GraphWorkerClient = class {
    ...
    constructor(worker_path) {
        ...
        this.client_services = {};
        this.bind_complete_resolve = null;
    }

    get_page_relative_url(url) {
    ...
    bind(skadi) {
        return new Promise((resolve,reject) => {
            this.bind_complete_resolve = resolve;
            this.skadi = skadi;
            ...
            if (!this.worker_path.endsWith(".js")) {
                // running in developer mode, import the scripts individually into the worker
                imports.push("../utils/index_db.js", "../utils/client_storage.js", "../utils/expr_parser.js",
                    "../core/node_execution_states.js", "../core/status_states.js", "../page/page_service.js",
                    "../plugins/topology_store.js")
            ...
            // add imports of any resources
            let resource_urls = skadi.get_core().get_schema().get_resource_urls();
            resource_urls.forEach((url) => imports.push(this.get_page_relative_url(url)));
            ...
            this.skadi.get_package_list().forEach(package_id => {
                ...
                packages.push({
                    ...
                    "base_url": this.get_page_relative_url(package_type.get_base_url())
                });
            });
            this.send({
                "action": "init",
                "imports": imports,
                "packages": packages,
                "workspace_id": skadi.get_workspace_id(),
                "class_map": class_map,
                "topology_id": skadi.get_core().get_id()
            });
    ...
    handle(control_packet, extras) {
        ...
        switch(action) {
            case "init_complete":
                this.bind_complete_resolve(true);
                break;
            case "client_message":
