...
skadi.Application = class  {
    ...
    create_configuration_page(div,configuration,page_id,show_title,show_status) {
        ...
        let pf = new skadi.PageFrame(configuration,page_id,page.title,url,div,show_title,show_status);
    ...
    create_node_page(div,node,page_id,show_title,show_status) {
        ...
        let pf = new skadi.PageFrame(node,page_id,page_name,url,div,show_title,show_status);
...
skadi.ApplicationApi = class extends skadi.Api {
     ...
     */
    async init(topology_id, options, plugins) {
        ...
        let core = new skadi.Core(this.workspace_id, topology_id, l10n_utils, schema, instance_factory || new skadi.InstanceFactory(), this.topology_store);

        await core.init();
        this.set_core(core);

        this.application = new skadi.Application(core);

        await engine.bind(this);

        this.topology_store.bind(this);
    }
    ...
    open_node_page(parent_elt_id, node_id, page_id, show_title, show_status) {
        let node = this.application.get_node(node_id);
        let x3_elt = skadi.$$$(parent_elt_id);
        this.application.create_node_page(x3_elt, node, page_id, show_title, show_status);
    }

    open_configuration_page(parent_elt_id, package_id, page_id, show_title, show_status) {
        let configuration = this.application.get_configuration(package_id);
        let x3_elt = skadi.$$$(parent_elt_id);
        this.application.create_configuration_page(x3_elt, configuration, page_id, show_title, show_status);
    }

    auto_tile(configuration_container_id, node_container_id, cell_width, cell_height) {
        let node_ids = this.get_node_ids();
        let configuration_ids = this.get_configuration_ids();
        let configuration_container = document.getElementById(configuration_container_id);
        let node_container = document.getElementById(node_container_id);

        for (let idx = 0; idx < configuration_ids.length; idx++) {
            let page_ids = this.get_page_ids_for_configuration(configuration_ids[idx]);
            for (let pidx = 0; pidx < page_ids.length; pidx++) {
                let div = document.createElement("div");
                div.style.width = cell_width + "px";
                div.style.height = cell_height + "px";
                let id = configuration_ids[idx] + "_" + page_ids[idx];
                div.setAttribute("id", id);
                configuration_container.appendChild(div);
                this.open_configuration_page(id, configuration_ids[idx], page_ids[pidx], true, true);
            }
        }

        for (let idx = 0; idx < node_ids.length; idx++) {
            let page_ids = this.get_page_ids_for_node(node_ids[idx]);
            for (let pidx = 0; pidx < page_ids.length; pidx++) {
                let div = document.createElement("div");
                div.style.width = cell_width + "px";
                div.style.height = cell_height + "px";
                let id = node_ids[idx] + "_" + page_ids[idx];
                div.setAttribute("id", id);
                node_container.appendChild(div);
                this.open_node_page(id, node_ids[idx], page_ids[pidx], true, true);
            }
        }
    }
}
  ...
  *
  * @param {string} workspace_id - the workspace id of the topology to be edited
  * @param {string} topology_id - the id of the topology to be edited
  * @param {Object} options - various options to customise skadi
  * @param {Object} plugins - various plugins to customise skadi
  */
skadi.start_application = async function(topology_id, options, plugins) {
    let skadi_instance = new skadi.ApplicationApi();
    await skadi_instance.init(topology_id, options, plugins || {});
