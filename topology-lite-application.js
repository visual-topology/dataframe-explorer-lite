...
/* src/js/topology-application-lite.js */

visualtopology_lite.TopologyApplicationLite = class extends visualtopology_lite.TopologyLite {
    ...
    async init() {
        await this.create_application();
        this.graph_worker_client.bind(this.application);
    }
    ...
    async create_application() {
        ...
        platform_extensions.push({"name":"Visual Topology Lite", "version": this.get_version(), "url":"https://github.com/visual-topology/visual-topology-lite"})
        this.options.platform_extensions = platform_extensions;

        this.application = await skadi.start_application(this.topology_id, this.container_id, this.options, plugins);
    ...
    async load_from_file(file, node_renamings) {
        await this.application.load_from(file, node_renamings);
    }
    ...
    get_page_ids_for_node(node_id) {
        return this.application.get_page_ids_for_node(node_id);
    }

    get_page_ids_for_configuration(package_id) {
        return this.application.get_page_ids_for_configuration(package_id);
    }

    open_node_page(parent_node_id, node_id, page_id, show_title, show_status, close_callback) {
        return this.application.open_node_page(parent_node_id, node_id, page_id, show_title, show_status, close_callback);
    }

    open_configuration_page(parent_node_id, configuration_id, page_id, show_title, show_status, close_callback) {
        return this.application.open_configuration_page(parent_node_id, configuration_id, page_id, show_title, show_status, close_callback);
    }
}

visualtopology_lite.TopologyApplicationLite.remove_topology = async function(topology_id) {
    let td = new visualtopology_lite.TopologyDirectory();
    await td.remove_topology(topology_id);
}
